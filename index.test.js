...
const createTestRepo = (inputs) => {
  ...
  return {
    clean: () =>
      execute(
        os.tmpdir(),
        windows ? `rmdir /s /q ${repoDirectory}` : `rm -rf ${repoDirectory}`,
      ),
    makeCommit: (msg, path) => {
      if (windows) {
        run(
          `fsutil file createnew ${path !== undefined ? path.trim("/") + "/" : ""}test${i++} 0`,
        );
      } else {
        run(
          `touch ${path !== undefined ? path.trim("/") + "/" : ""}test${i++}`,
        );
      }
      run(`git add --all`);
    ...
    runAction: (inputs) =>
      run(`node ${path.join(__dirname, "index.js")}`, inputs),
    exec: run,
  };
};
...
test("Format input is respected", () => {
  const repo = createTestRepo({
    format: "M${major}m${minor}p${patch}i${increment}",
  }); // M0m0p0i0
...
test("[Use Branch Names] Commit messages do not affect version", () => {
  ...
  const repo3 = createTestRepo({
    tag_prefix: "",
    use_test_value: true,
    test_value: "minor/problem",
    major_pattern: "major/",
    minor_pattern: "minor/",
  }); // 0.0.0
...
test("[Use Branch Names] regex works as expected", () => {
  const repo = createTestRepo({
    ...
    minor_pattern: "/^(minor|feature|feat)//",
  }); // 0.0.0
  repo.makeCommit("Initial Commit");
  repo.exec("git tag 1.2.3");
  ...
  const repo4 = createTestRepo({
    tag_prefix: "",
    use_test_value: true,
    test_value: "breaking/banger",
    major_pattern: "/^(major|breaking)/",
    minor_pattern: "/^(minor|feature|feat)//",
  }); // 0.0.0
  ...
  const repo5 = createTestRepo({
    tag_prefix: "",
    use_test_value: true,
    test_value: "feature/problem",
    major_pattern: "/^(major|breaking)/",
    minor_pattern: "/^(minor|feature|feat)//",
  }); // 0.0.0
  ...
  const repo6 = createTestRepo({
    tag_prefix: "",
    use_test_value: true,
    test_value: "feat/problem",
    major_pattern: "/^(major|breaking)/",
    minor_pattern: "/^(minor|feature|feat)//",
  }); // 0.0.0
  ...
  const repo7 = createTestRepo({
    tag_prefix: "",
    use_test_value: true,
    test_value: "team/feat/bar",
    major_pattern: "/^(major|breaking)/",
    minor_pattern: "/^(minor|feature|feat)//",
  }); // 0.0.0
  ...
  repo7.makeCommit("Second commit on patch branch");
  expect(repo7.runAction()).toMatch("Version is 1.2.4+1");

  const repo8 = createTestRepo({
    tag_prefix: "",
    use_test_value: true,
    test_value: "team/feat/bar",
    major_pattern: "/^(major|breaking)/",
  ...
  repo8.makeCommit("Initial Commit");
  repo8.exec("git tag 1.2.3");
  repo8.exec("git switch -c some-branch");

  repo8.makeCommit("First commit on patch branch");
  repo8.makeCommit("Second commit on patch branch");
  expect(repo8.runAction()).toMatch("Version is 1.2.4+1");
