...
const setOutput = (
  ...
  if (repository !== undefined && !namespace) {
    core.info(
      `To create a release for this version, go to https://github.com/${repository}/releases/new?tag=${tag}&target=${branch.split("/").reverse()[0]}`,
    );
  }
...
const parseVersion = (tag) => {
  ...
  if (tagPrefix.includes("/") && tag.includes(tagPrefix)) {
    let tagParts = tag.replace(tagPrefix, "<--!PREFIX!-->").split("/");
    stripedTag = tagParts[tagParts.length - 1].replace(
      "<--!PREFIX!-->",
      tagPrefix,
    );
  } else {
...
async function run() {
  try {
    ...
    const majorPattern = createMatchTest(
      core.getInput("major_pattern", { required: true }),
    );
    const minorPattern = createMatchTest(
      core.getInput("minor_pattern", { required: true }),
    );
    ...
    let currentTag = (
      await cmd(`git tag --points-at ${versionBranch} ${releasePattern}`)
    ).trim();
    ...
    const rawLog = await cmd(logCommand);
    ...
    if (changePath !== "") {
      if (root === "") {
        ...
        core.info(
          `No tags found, checking all commits with command: ${command}`,
        );
    ...
    core.info(
      `Changed: ${changed} ${changed.toString()} for ${changePath} [${versionBranch}]`,
    );

    let history = log.trim().split(eol).reverse();

    let rawHistory = rawLog.trim().split(eol).reverse();
